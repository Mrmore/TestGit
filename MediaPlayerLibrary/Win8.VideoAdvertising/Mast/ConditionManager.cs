...
namespace Microsoft.VideoAdvertising
{
    ...
    internal class ConditionManager : IDisposable
    {
        ...
        public ConditionManager(Condition condition, IMastAdapter mastInterface)
        {
            ...
            {
#if SILVERLIGHT
                var eventHandler = ReflectionHelper.AttachEvent(MastInterface, condition.Name, this, "OnEventFired");
                eventUnhookAction = () => ReflectionHelper.DetachEvent(MastInterface, eventHandler);
#else
        ...
        private bool ModTimeSpans(TimeSpan prop, TimeSpan val)
        {
            ...
            if (prop > TimeSpan.FromMilliseconds(100))
            {
                ...
                Task
#endif
                .Delay(prop, cancellationToken.Token).ContinueWith(t =>
                {
                    if (t.IsCompleted)
                    {
                        DoOnEventFired();
