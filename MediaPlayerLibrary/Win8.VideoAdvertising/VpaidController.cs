...
#if SILVERLIGHT
        void player_IsMutedChanged(object sender, EventArgs e)
#else
...
#if SILVERLIGHT
        void player_DimensionsChanged(object sender, EventArgs e)
#else
            ...
            foreach (var trackingEvent in adUnit.CreativeSource.TrackingEvents.Where(te => te.Type == eventToTrack))
            {
                ...
                switch (trackingEvent.Type)
                {
                    ...
                    case TrackingType.Complete:
                        ...
                        url.Replace(Macro_PreviousQuartile, previousEvents.Any() ? ((int)Math.Round(currentTime.Subtract(previousEvents.Last()).TotalSeconds)).ToString() : "0");
