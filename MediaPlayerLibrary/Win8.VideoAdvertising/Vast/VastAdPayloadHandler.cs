...
namespace Microsoft.VideoAdvertising
{
    public sealed class VastAdPayloadHandler : IAdPayloadHandler
    {
        ...
        static async Task ProcessPayload(IAdSource adSource)
        {
            ...
            {
                adSource.Payload = await AdModelFactory.CreateFromVast((Stream)adSource.Payload, adSource.MaxRedirectDepth, adSource.AllowMultipleAds);
            }
            else if (adSource.Payload is string)
            {
                adSource.Payload = await AdModelFactory.CreateFromVast(new MemoryStream(System.Text.Encoding.UTF8.GetBytes((string)adSource.Payload)), adSource.MaxRedirectDepth, adSource.AllowMultipleAds);
            }
...
#if SILVERLIGHT
        public Task<bool> CancelAd()
        {
            return adHandlerBase.CancelAd();
        }
#else
