...
namespace Microsoft.VideoAdvertising
{
    public static partial class AdModelFactory
    {
        ...
        private static void MergeWrappedAd(Ad wrapperAd, Ad inlineAd)
        {
            // carry over impressions
            foreach (var impression in wrapperAd.Impressions)
                inlineAd.Impressions.Add(impression);

            foreach (var error in wrapperAd.Errors)
                inlineAd.Errors.Add(error);

            // carry over tracking events
            foreach (ICreative creative in inlineAd.Creatives)
            {
                ICreative creativeWrapper = FindMatchingCreative(inlineAd, creative, wrapperAd);
                if (creativeWrapper != null)
                {
                    if (creative is CreativeLinear)
                    {
                        var linearWrapper = (CreativeLinear)creativeWrapper;
                    ...
                    {
                        var nonLinearsWrapper = (CreativeNonLinears)creativeWrapper;
                    ...
                    {
                        var companions = (CreativeCompanions)creative;
                        var companionsWrapper = (CreativeCompanions)creativeWrapper;
                            ...
                            case "Linear":
                                ...
                                if (vastLinear.Elements("MediaFiles").Any())
                                {
                                    foreach (var vastMedia in vastLinear.Element("MediaFiles").Elements("MediaFile"))
                                    {
                                        ...
                                        mediaFile.ApiFramework = (string)vastMedia.Attribute("apiFramework");
                                        mediaFile.Bitrate = ToNullableLong((string)vastMedia.Attribute("bitrate"));
                                        mediaFile.Codec = (string)vastMedia.Attribute("codec");
...
#if WINDOWS_PHONE
            if (EnumEx.TryParse<TrackingType>(eventName, true, out trackingType))
#else
        ...
        {
#if WINDOWS_PHONE
            return System.Net.HttpUtility.HtmlDecode(value);
#elif SILVERLIGHT
            return System.Windows.Browser.HttpUtility.HtmlDecode(value);
#else
