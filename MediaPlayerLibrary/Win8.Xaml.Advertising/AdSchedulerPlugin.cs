...
namespace Microsoft.PlayerFramework.Advertising
{
    ...
    public partial class AdSchedulerPlugin : PluginBase
    {
        ...
        {
            EvaluateMarkers(e.PreviousPosition, e.Position, false);
        }
        ...
        void MediaPlayer_Scrubbing(object sender, ScrubProgressRoutedEventArgs e)
        {
            if (InterruptScrub)
            {
                e.Canceled = EvaluateMarkers(e.StartPosition, e.Position, true);
            }
        }
        ...
        void MediaPlayer_ScrubbingCompleted(object sender, ScrubProgressRoutedEventArgs e)
        {
            if (!e.Canceled)
            {
                e.Canceled = EvaluateMarkers(e.StartPosition, e.Position, InterruptScrub);
            }
        }
        ...
        /// <param name="syncPosition">Determines if the main content should seek to the ad spot in the event that an ad is triggered.</param>
        /// <returns>A boolean indicating that an ad was played.</returns>
        public bool EvaluateMarkers(TimeSpan originalPosition, TimeSpan newPosition, bool syncPosition)
        {
            if (!EvaluateOnForwardOnly || newPosition > originalPosition)
            {
                foreach (var marker in MediaPlayer.Markers.Where(m => m.Type == MarkerType_Play).ToList())
                {
                    ...
                    {
                        return PlayAdMarker(marker, syncPosition);
                    }
                }
            }
            return false;
...
#endif
        {
            ...
            {
                if (EligableAds.OfType<PrerollAdvertisement>().Any(a => a.Source != null))
                {
                    var deferral = e.DeferrableOperation.GetDeferral();
                    try
                    {
                        await PlayAdsOfType<PrerollAdvertisement>(CancellationTokenSource.CreateLinkedTokenSource(deferral.CancellationToken, cts.Token).Token);
                    }
                    catch { /* ignore */ }
                    finally
                    {
        ...
        bool PlayAdMarker(TimelineMarker marker, bool syncPosition)
        {
            var ad = EligableAds.OfType<Advertisement>().Where(a => a.Source != null).FirstOrDefault(a => a.Id == marker.Text);
            if (ad != null)
            {
                ...
                if (syncPosition)
                {
                    MediaPlayer.Position = marker.Time;
                }
                return true;
            }
            return false;
