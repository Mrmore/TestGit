    ...
    public class FreeWheelPlugin : AdSchedulerPlugin
    {
        ...
        public async Task LoadAds(Uri source, CancellationToken c)
        {
            ...
            if (videoTracking != null)
            {
                ...
                positionTrackingPlugin.TrackingEvents.Add(new PositionTrackingEvent() { PositionPercentage = 0, Data = videoTracking });
                positionTrackingPlugin.TrackingEvents.Add(new PositionTrackingEvent() { PositionPercentage = 1, Data = videoTracking });
                ...
                for (int i = 15; i < 60; i = i + 15)
                    playTimeTrackingPlugin.TrackingEvents.Add(new PlayTimeTrackingEvent() { PlayTime = TimeSpan.FromSeconds(i), Data = videoTracking });
            ...
            if (videoAsset != null)
            {
                foreach (var adSlot in videoAsset.AdSlots)
                {
                    ...
                    var payload = FreeWheelFactory.GetAdDocumentPayload(adSlot, adResponse);
